  ...
  describe "Source filtering" $ do
    ...
    it "includes a source" $ withTestEnv $
      searchExpectSource
        ...
        (Right (Object (KeyMap.fromList [("message", String "Use haskell!")])))

    it "includes sources" $ withTestEnv $
      searchExpectSource
        (SourcePatterns (PopPatterns [Pattern "user", Pattern "message"]))
        (Right (Object (KeyMap.fromList [("user",String "bitemyapp"),("message", String "Use haskell!")])))

    it "includes source patterns" $ withTestEnv $
      searchExpectSource
        (SourcePatterns (PopPattern (Pattern "*ge")))
        (Right (Object (KeyMap.fromList [("age", Number 10000),("message", String "Use haskell!")])))
    ...
    it "excludes source patterns" $ withTestEnv $
      searchExpectSource
        ...
        (Right (Object (KeyMap.fromList [("user",String "bitemyapp")])))
