...
instance ToJSON Suggest where
  toJSON Suggest{..} =
    object [ "text" .= suggestText
           , Key.fromText suggestName .= suggestType
           ]
...
instance FromJSON Suggest where
  parseJSON (Object o) = do
    ...
    let dropTextList =
            KeyMap.toList
          $ KeyMap.filterWithKey (\x _ -> x /= "text") o
    suggestName' <-
      case dropTextList of
        [(x, _)] -> return x
        _ -> fail "error parsing Suggest field name"
    suggestType' <- o .: suggestName'
    return $ Suggest suggestText' (Key.toText suggestName') suggestType'
...
instance FromJSON NamedSuggestionResponse where
  parseJSON (Object o) = do
    suggestionName' <- case KeyMap.toList o of
                        [(x, _)] -> return x
                        _ -> fail "error parsing NamedSuggestionResponse name"
    suggestionResponses' <- o .: suggestionName'
    return $ NamedSuggestionResponse (Key.toText suggestionName') suggestionResponses'
